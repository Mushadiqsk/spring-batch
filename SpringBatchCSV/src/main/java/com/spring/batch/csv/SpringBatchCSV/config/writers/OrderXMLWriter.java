...
public class OrderXMLWriter<Order> extends StaxEventItemWriter<Order>{
	 ...
	 */
	@Override
	public void write(List<? extends Order> items) throws XmlMappingException, IOException {
		...
		items.stream().forEach(order->{;
			Assert.state(marshaller.supports(order.getClass()),
